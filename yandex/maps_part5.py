...
def fetch_map(ll: str, spn: float, theme: str = THEME_LIGHT, pt: str | None = None) -> bytes | None:
    params = {
        "ll": ll,
        "spn": f"{spn},{spn}",
...
class MapWindow(QMainWindow):
    ...
    def _load_map(self):
        ...
        data = fetch_map(self._ll_str(), self.spn, self.theme, self._pt_str())
        if data is None:
        ...
        marker_info = f"   Метка: {self.marker[0]:.6f},{self.marker[1]:.6f}" if self.marker else ""
        self.status.showMessage(
            f"Центр: {self._ll_str()}   Масштаб: {self.spn}   Тема: {theme_label}{marker_info}"
        )
