...
DEFAULT_SPN = "0.05,0.05"


def fetch_map(ll: str, spn: str) -> bytes | None:
    params = {
        "ll": ll,
        "spn": spn,
...
class MapWindow(QMainWindow):
    def __init__(self, ll: str = DEFAULT_LL, spn: str = DEFAULT_SPN):
        super().__init__()
        self.ll = ll
        self.spn = spn
    ...
    def _build_ui(self):
        ...
        self.spn_edit = QLineEdit(self.spn)
        self.spn_edit.setPlaceholderText("0.05,0.05")
    ...
    def _on_show(self):
        ...
        self.spn = self.spn_edit.text().strip()
        self._load_map()
...
def main():
    start_ll = DEFAULT_LL
    start_spn = DEFAULT_SPN

    app = QApplication(sys.argv)
    window = MapWindow(ll=start_ll, spn=start_spn)
    window.show()
    sys.exit(app.exec())
